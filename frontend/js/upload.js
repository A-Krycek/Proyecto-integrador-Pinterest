        ...
        $formSubida.on("submit", async function(e) {
            ...
            try {
                ...
                if (pin.status === "approved") {
                    ...
                    $btnPublicar.prop("disabled", false).text("Publicar");
                    showToast("Contenido Rechazado", "La publicación fue rechazada: " + (pin.moderation_reason || "Contenido no permitido en LookBook."), "error");
                } else if (pin.status === "sensitive") {
                    $btnPublicar.prop("disabled", false).text("Publicar");
                    showToast(
                        "⚠️ Contenido Sensible Detectado",
                        "Tu publicación ha sido marcada como contenido sensible y está en espera de revisión manual. " +
                        (pin.moderation_reason || "Revisa las Políticas de Comunidad de LookBook."),
                        "warning"
                    );
                } else {
